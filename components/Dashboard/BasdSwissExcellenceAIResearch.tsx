        ...
        <div className="flex gap-16 mx-auto my-20 max-w-[1500px] relative text-center">
            <div className="py-8 w-1/2 sticky top-0 [height:fit-content]">
                <div className="relative w-[50%]">
                    <div className="text-3xl font-bold my-8 w-[90%] mx-auto">Based on Swiss Excellence in AI Research</div>
                    <div className="text-xl text-neutral-500 mt-4">
                    ...
                    {imageTexts.map((text, index) => (
                        <div
                            key={index}
                            className={`transition-all duration-700 absolute top-0 left-[80%] w-[90%] text-3xl [transform:translate(50%_,50%_)] rounded-lg p-4 bg-gray-100 text-black ${activeIndex === index
                                    ? 'opacity-100 transition-y-0'
                                    : 'opacity-0 transition-y-4'
                                }`}
                        >
                            <div className="absolute top-[50%] left-full -mt-2 border-l-[10px] border-t-[5px] border-b-[15px] border-solid border-transparent border-l-white"></div>
                            {text.description}
                        </div>
                    ))}
                </div>
            </div>
            <div className="flex flex-col gap-8 px-8 py-16 w-1/2 bg-[#101010] rounded-lg">
                {[
